...
void x64::init_registers()
{
	...
	registers.insert({ RBX, new reg { .id = RBX, .general = true,  .volatil = true } });
	...
	registers.insert({ RSI, new reg { .id = RSI, .general = true,  .volatil = true } });
	registers.insert({ RDI, new reg { .id = RDI, .general = true,  .volatil = true } });
	...
	registers.insert({ R12, new reg { .id = R12, .general = true,  .volatil = true } });
	registers.insert({ R13, new reg { .id = R13, .general = true,  .volatil = true } });
	registers.insert({ R14, new reg { .id = R14, .general = true,  .volatil = true } });
	registers.insert({ R15, new reg { .id = R15, .general = true,  .volatil = true } });
}
